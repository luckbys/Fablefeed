...
class FableFeedSupabaseUser extends BaseAuthUser {
  FableFeedSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fableFeedSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FableFeedSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
